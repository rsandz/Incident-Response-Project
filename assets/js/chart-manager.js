...
class chartManager {
  ...
  generateData(data) {
    ...
    for (var i = dataDates.length - 1; i >= 0; i--) {
      dataDates[i].setMinutes(
        dataDates[i].getMinutes() + dataDates[i].getTimezoneOffset()
      );
    }
    switch (this.type) {
      case "daily":
        for (var i = this.limit - 1 + this.offset; i >= this.offset; i--) {
          ...
          let index = dataDates.findIndex(
            x => x.toDateString() == currentDate.toDateString()
          );
          if (index == -1) {
            dataArray.push(0);
          } else {
  ...
  generateChartData() {
    ...
    switch (this.type) {
      ...
      case "weekly":
        ...
        break;
      case "monthly":
        var label_array = chartData[0].dateArray.map(
  ...
  getBackgroundColours() {
    if (!this.backgroundColours) {
      ...
      for (let index in this.ajaxURLs) {
        this.backgroundColours.push(
          "hsla(" + Math.random() * 360 + ", 100%, 77%, 0.8)"
        );
      }
    }
  ...
  getData(callback) {
    if (Array.isArray(this.ajaxURLs)) {
      ...
      for (let url of this.ajaxURLs) {
        requests.push(
          $.get(url, { interval_type: this.type }, data =>
            this.dataList.push(JSON.parse(data))
          )
        );
      }
      $.when(...requests).then(callback.bind(this));
    } else {
      $.get(this.ajaxURLs, { interval_type: this.type }, data =>
        this.dataList.push(JSON.parse(data))
   ...
   */
  searchForData(
    e //For loading the graph search
  ) {
    try {
  ...
  jumpTo(jumpDate) {
    if (jumpDate) {
      ...
      switch (this.type) {
        ...
        case "weekly":
          offset = Math.round(
            Math.round((Date.now() - jumpDate) / 86400000) / 7
          ); //Get day offset and divide by 7!
          break;
        case "monthly":
